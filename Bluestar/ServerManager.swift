...
class ServerManager {
    ...
    func getUserDetails(userName: String, completion: @escaping (_ result: String, _ data: Data) -> Void)  {
        ...
        let urlString = "http://14.142.218.42/LMS/service.asmx"
...
//            let soapMessage = "<?xml version='1.0' encoding='utf-8'?><soap:Envelope xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema' xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'><soap:Body><createLead xmlns='http://tempuri.org/'><CustomerName>\(leadDetails["CustomerName"]!)</CustomerName><EmailID>\(leadDetails["EmailID"]!)</EmailID><MobileNumber>\(leadDetails["MobileNumber"]!)</MobileNumber><AlternateNumber>\(leadDetails["AlternateNumber"]!)</AlternateNumber><Pincode>\(leadDetails["Pincode"]!)</Pincode><CityName>\( leadDetails["CityName"]!)</CityName><Address>\(leadDetails["Address"]!)</Address><LeadSource>\(leadDetails["LeadSource"]!)</LeadSource><LeadSubSource>\(leadDetails["LeadSubSource"]!)</LeadSubSource><ProductName>\(leadDetails["ProductName"]!)</ProductName><ModelName>\(leadDetails["ModelName"]!)</ModelName><SwcName >\(leadDetails["SwcName"]!)</SwcName><DemoFixedDate>\(leadDetails["DemoFixedDate"]!)</DemoFixedDate><FollowUpDate>\(leadDetails["FollowUpDate"]!)</FollowUpDate><LeadStatus>\(leadDetails["LeadStatus"]!)</LeadStatus><Comments>\(leadDetails["Comments"]!)</Comments><RoleID>\(leadDetails["RoleID"]!)</RoleID><CreatedBy>\(leadDetails["CreatedBy"]!)</CreatedBy><Longitude>\(leadDetails["Longitude"]!)</Longitude><Latitude>\(leadDetails["Latitude"]!)</Latitude><LeadRaisedAddress>\(leadDetails["LeadRaisedAddress"]!)</LeadRaisedAddress></createLead></soap:Body></soap:Envelope>"
            let urlString = "http://14.142.218.42/LMS/service.asmx"
    ...
    func updateLead(leadDetails: [String: String], completion: @escaping (_ result: String, _ data: Data) -> Void)  {
        ...
        if let seriesNumber = leadDetails["SeriesNumber"], let customerName = leadDetails["CustomerName"], let emailID = leadDetails["EmailID"], let mobileNumber = leadDetails["MobileNumber"], let alternateNumber = leadDetails["AlternateNumber"], let pincode = leadDetails["Pincode"], let cityName = leadDetails["CityName"], let address = leadDetails["Address"] , let leadSource = leadDetails["LeadSource"] , let productName = leadDetails["ProductName"] , let modelName = leadDetails["ModelName"] , let swcName = leadDetails["SwcName"] , var demoFixedDate = leadDetails["DemoFixedDate"] , let leadStatus = leadDetails["LeadStatus"] , let comments = leadDetails["Comments"],  let createBy = leadDetails["UpdatedBy"] , let longitude = leadDetails["Longitude"] , let Latitude = leadDetails["Latitude"] , let leadRaisedAddress = leadDetails["LeadRaisedAddress"] {
            let leadSubSource = ""
            var followUpDate = ""
            if let followUp = leadDetails["FollowUpDate"] as? String {
                followUpDate = followUp
            }

            if demoFixedDate.characters.count > 0 {
                demoFixedDate = demoFixedDate.appending(" 12:00:00 AM")
            }
            
            let soapMessage = "<soapenv:Envelope xmlns:soapenv='http://schemas.xmlsoap.org/soap/envelope/' xmlns:tem='http://tempuri.org/'><soapenv:Header/><soapenv:Body><tem:updateLead>< SeriesNumber >\(seriesNumber)</tem: SeriesNumber ><CustomerName>\(customerName)</CustomerName><EmailID>\(emailID)/ EmailID><MobileNumber>\(mobileNumber)</ MobileNumber><AlternateNumber>\(alternateNumber)</AlternateNumber><Pincode>\(pincode)</ Pincode><CityName>\(cityName)</ CityName><Address>\(address)</ Address><LeadSource>\(leadSource)</ LeadSource><LeadSubSource>\(leadSubSource)</ LeadSubSource><ProductName>\(productName)</ ProductName><SwcName>\(swcName)</SwcName><ModelName>\(modelName)</ ModelName><DemoFixedDate>\(demoFixedDate)</ DemoFixedDate><FollowUpDate>\(followUpDate)</ FollowUpDate><LeadStatus>\(leadStatus)</LeadStatus><Comments>\(comments)/ Comments><CreatedBy>\(createBy)</ CreatedBy><Longitude>\(longitude)</Longitude><Latitude>\(Latitude)</Latitude><LeadRaisedAddress>\(leadRaisedAddress)</LeadRaisedAddress></ updateLead></soapenv:Body></soapenv:Envelope>"
//            let soapMessage = "<?xml version='1.0' encoding='utf-8'?><soap:Envelope xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema' xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'><soap:Body <updateLead xmlns='http://tempuri.org/'><SeriesNumber>\(seriesNumber)</SeriesNumber><CustomerName>\(customerName)</CustomerName><EmailID>\(emailID)</EmailID><MobileNumber>\(mobileNumber)</MobileNumber><AlternateNumber>\(alternateNumber)</AlternateNumber><Pincode>\(pincode)</Pincode><CityName>\(cityName)</CityName><Address>\(address)</Address><LeadSource>\(leadSource)</LeadSource><LeadSubSource>\(leadSubSource)</LeadSubSource><ProductName>\(productName)</ProductName><ModelName>\(modelName)</ModelName><SwcName >\(swcName)</SwcName><DemoFixedDate>\(demoFixedDate)</DemoFixedDate><FollowUpDate>\(followUpDate)</FollowUpDate><LeadStatus>\(leadStatus)</LeadStatus><Comments>\(comments)</Comments><RoleID>\(roleID)</RoleID><UpdatedBy>\(createBy)</UpdatedBy><Longitude>\(longitude)</Longitude><Latitude>\(Latitude)</Latitude><LeadRaisedAddress>\(leadRaisedAddress)</LeadRaisedAddress></updateLead></soap:Body></soap:Envelope>"
